...
export default function NFSEDoc({ isOpen, onClose, onConfirm }) {
  ...
  />
        ...
        {modo === "manual" && (
          <div className="grid grid-cols-3 gap-4 p-3 border rounded bg-gray-50 mb-4">
            <div>
              <Label>Qtd</Label>
              <Txt type="number" value={qtd} onChange={(e) => setQtd(Number(e.target.value))} />
            </div>

            <div>
              <Label>Valor</Label>
              <Txt value={valor} onChange={(e) => setValor(e.target.value)} />
            </div>

            <div>
              <Label>Total</Label>
              <Txt value={total} readOnly className="bg-gray-100" />
            </div>
          </div>
        )}
